...
import {Edit3Icon, TagIcon, PlusIcon, MinusIcon, RotateCcwIcon, RotateCwIcon} from 'vue3-feather';
...
function removeFilterStep(i) {
	filterSteps.update([...filterSteps.steps.value.slice(0, i), ...filterSteps.steps.value.slice(i+1)]);
}

function updateFilterStep(i, step) {
	filterSteps.update([...filterSteps.steps.value.slice(0, i), step, ...filterSteps.steps.value.slice(i+1)]);	
}
...
const lineCounts = computed(() => {
	return {
		...
		'clean': samples.value.length === filterSteps.steps.value.length + 1 ? samples.value[filterSteps.steps.value.length].stdout?.length : null,
...
const filterIsOpen = new class {
	...
	get(filterStep) {
		if (!this.map.has(filterStep.id))
			this.map.set(filterStep.id, ref(false));
			...
			<header class="controls">
				...
				<SegmentedControl class="table-buttons" v-model="view" :options="VIEWS">
					<template v-slot="{option}">
						{{ option }} <small class="line-count" title="Number of lines in sample" v-if="lineCounts[option] !== null">{{ lineCounts[option] }}</small>
					</template>
				</SegmentedControl>
			</header>
