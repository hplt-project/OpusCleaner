...
def parse_user_args():
    ...
    parser.add_argument(
        "--src-lang", default="en", type=str, choices=list(CHARS.keys())
    )
...
def clean_parallel(
    src_lang: str,
    ratio_words_src: float,
    ratio_alpha_src: float,
    trg_lang: Optional[str],
    ratio_words_trg: float,
    ratio_alpha_trg: float,
    debug: bool = True,
) -> None:
    ...
    for line in stdin:
        ...
        if src_lang in CHARS:
            ...
            num_words = sum(
                [
                    1 if re.match(CHARS[src_lang], t, re.IGNORECASE) else 0
                    for t in src_toks
                ]
            )
        ...
        if trg is not None and trg_lang in CHARS:
            ...
            num_words = sum(
                [
                    1 if re.match(CHARS[trg_lang], t, re.IGNORECASE) else 0
                    for t in trg_toks
                ]
            )
...
if __name__ == "__main__":
    ...
    clean_parallel(
        ...
        ratio_words_src=args.ratio_words_src,
        ratio_alpha_src=args.ratio_alpha_src,
        trg_lang=args.trg_lang,
        ratio_words_trg=args.ratio_words_trg,
        ratio_alpha_trg=args.ratio_alpha_trg,
        debug=args.debug,
    )
