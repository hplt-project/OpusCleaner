...
def normalize(numstr: Match) -> str:
    return (numstr["sign"] or "") + re.sub(
        r"[^\d]+", "*", numstr["value"]
    )  # ignore the decimal and digit separators


def filter_numerical_mismatch(
    fin: TextIO, fout: TextIO, ratio: float, *, debug: bool = False
):
    for line in fin:
        ...
        nums_left, nums_right = (
            set(map(normalize, re.finditer(NUM_EXPR, col))) for col in cols[:2]
        )
        ...
        if nums_left or nums_right:
            ...
            if debug:
                print(
                    f"{len(overlap)} / {len(difference)} : {overlap!r} | {difference!r}",
                    file=sys.stderr,
                )
