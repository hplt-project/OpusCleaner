...
class Span:
    ...
    def __enter__(self) -> "Span":
        self.span = self.logger.push(
            self.name, **self.extra, type="span", start=time.monotonic_ns()
        )
        return self
...
class Logger:
    ...
    def update(self, event_id: UUID, **kwargs) -> None:
        ...
        self.handler.emit({"id": event_id, **kwargs})
...
class Context:
    ...
    def __init__(self, *, file: Optional[IO[str]] = None):
        ...
        self.loggers = {_main_thread_id: Logger(self.handler)}
    ...
    def get_logger(self):
        ...
        if thread_id not in self.loggers:
            ...
            self.loggers[thread_id].stack = deque(
                self.loggers[_main_thread_id].stack
            )  # TODO what about threads starting threads?
