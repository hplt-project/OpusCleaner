...
def reservoir_sample(
    k: int, it: Iterable[T], *, rand: random.Random = random._inst, sort: bool = False
) -> List[T]:
...
class Tailer(Iterable[T]):
    ...
    def tail(self) -> List[T]:
        ...
        return (
            self.sample[(self.i % len(self.sample)) :]
            + self.sample[0 : (self.i % len(self.sample))]
        )
...
def gunzip(path: str) -> Iterator[IO[bytes]]:
    ...
    with subprocess.Popen(["gzip", "-cd", path], stdout=subprocess.PIPE) as proc:
        ...
        if proc.wait() != 0:
            raise RuntimeError(
                f"gzip returned error code {proc.returncode} while decompressing {path}"
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Take a file's head, tail and a random sample from the rest."
    )
    parser.add_argument(
        "-n",
        dest="lines",
        type=int,
        default=10,
        help="number of lines for each section of the sample",
    )
    parser.add_argument(
        "-d",
        dest="delimiter",
    ...
    parser.add_argument(
        "-N", "--line-numbers", action="store_true", help="print line numbers"
    )
    parser.add_argument(
        "files",
        metavar="file",
        ...
        nargs="*",
        default=["-"],
        help="files to sample. Multiple files for multiple columns. Use '-' for stdin. If none, reads from stdin.",
    )
    args = parser.parse_args()

    with ExitStack() as ctx:
        columns: List[Iterator[bytes]] = [
            magic_open_or_stdin(ctx, file) for file in args.files
        ]
