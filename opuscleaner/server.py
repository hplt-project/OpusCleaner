        ...
        for lineno, line in enumerate(
            ...
            for colno, field in enumerate(line.rstrip(b"\r").split(b"\t"), start=1):
                try:
                    ...
                    values.append(
                        f"[Error: Cannot decode line {lineno} column {colno}: {e!s}]"
                    )
            lines.append(dict(zip_longest(output.langs, values, fillvalue="")))

        super().__init__(
            returncode=output.returncode, stdout=lines, stderr=output.stderr.decode()
        )
...
async def get_dataset_sample(
    name: str, columns: List[Tuple[str, Path]]
) -> FilterOutput:
    langs = [lang for lang, _ in columns]
...
async def exec_filter_step(
    filter_step: FilterStep, langs: List[str], input: bytes
) -> Tuple[bytes, bytes]:
    ...
    p_filter = await asyncio.create_subprocess_shell(
        command,
...
def cancel_cached_tasks(name: str, offset: int) -> None:
    assert (
        offset > 0
    )  # offset == 0 is sample.py itself, and should never be cancelled through this method.
...
async def get_sample(
    name: str, filters: List[FilterStep]
) -> AsyncIterator[FilterOutput]:
    columns: List[Tuple[str, Path]] = list_datasets(DATA_PATH)[name]
    langs = [lang for lang, _ in columns]

    checksum = cache_hash(
        [(name, str(path), path.stat().st_mtime) for name, path in columns]
    )
    ...
    for i, filter_step in enumerate(filters, start=1):
        ...
        checksum = cache_hash(
            ...
            cache_hash(
                jsonable_encoder(filter_definition), sample_cache[name][i - 1].checksum
            ),
        )
        ...
        if len(sample_cache[name]) <= i or sample_cache[name][i].checksum != checksum:
            ...
            sample_cache[name].append(
                SampleCacheEntry(
                    checksum=checksum,
                    future=asyncio.create_task(
                        exec_filter_step(filter_step, langs, sample.stdout)
                    ),
                )
            )
...
def api_get_sample(name: str) -> Response:
    return stream_jsonl(
        ParsedFilterOutput(output) async for output in get_sample(name, [])
    )
...
def api_get_filtered_sample(name: str, filters: List[FilterStep]) -> Response:
    return stream_jsonl(
        ParsedFilterOutput(output) async for output in get_sample(name, filters)
    )
...
def make_pipeline(name: str, filters: List[FilterStep] = []) -> FilterPipeline:
    ...
    return FilterPipeline(
        version=1, files=[file.name for _, file in columns], filters=filters
    )
...
def api_get_dataset_filters_as_openfilter(name: str) -> Response:
    ...
    for step in pipeline.filters:
        if match := re.search(
            r"\bopusfilter\.preprocessors\.(\w+)\b",
            get_global_filter(step.filter).command,
        ):
            preprocess_steps.append({str(match.group(1)): step.parameters})
        elif match := re.search(
            r"\bopusfilter\.filters\.(\w+)\b", get_global_filter(step.filter).command
        ):
            filter_steps.append({str(match.group(1)): step.parameters})
            ...
            raise ValueError(
                f'Cannot convert "{step.filter}" to opusfilter configuration'
            )

    if preprocess_steps:
        output_files = [
            os.path.join(
                os.path.dirname(file), "preprocessed." + os.path.basename(file)
            )
    ...
    return Response(
        yaml.safe_dump(opusfilter_config, sort_keys=False),
...
async def sample_all_datasets(args):
    ...
    for task, result in zip(
        ...
        await asyncio.gather(
            *[compute_sample(*task) for task in tasks], return_exceptions=True
        ),
    ):
        if isinstance(result, Exception):
            print(
                f"Could not compute sample for {task[0]}: {result!s}", file=sys.stderr
            )
...
def main(argv=sys.argv):
    ...
    parser = argparse.ArgumentParser(
        description="Fill up those seats on your empty train."
    )
