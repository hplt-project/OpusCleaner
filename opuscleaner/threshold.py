...
class PersistentCache(Cache):
    ...
    def __init__(self, path: str):
        self.db = dbm.open(
            path, "c"
        )  # was 'cfu' create, fast, unlocked (TODO unlocked?!) but that only works if the gnu backend is used
...
def main():
    ...
    parser.add_argument(
        "threshold", type=float, help="Threshold (b) to compare score to."
    )
    parser.add_argument(
        "scorer", type=str, nargs="+", help="Scorer program (a) and arguments."
    )
    ...
    for name in ["lt", "le", "eq", "ne", "ge", "gt"]:
        ops.add_argument(
            ...
            dest="operator",
            action="store_const",
            const=getattr(operator, name),
            help=getattr(operator, name).__doc__,
        )
    ...
    try:
        with open_cache(args.cache) as cache:
            ...
            feeder = Thread(
                target=exit_on_throw(feed_child),
                args=[queue, sys.stdin.buffer, child.stdin, cache],
            )
            feeder.start()

            # Reads queue, writes to stdout, reading scores from child if necessary.
            consumer = Thread(
                target=exit_on_throw(threshold_scores),
                args=[
                    queue,
                    child.stdout,
                    sys.stdout.buffer,
