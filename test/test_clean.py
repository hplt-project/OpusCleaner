...
FILES = ["bible-uedin-v1.de-en.de.gz", "bible-uedin-v1.de-en.en.gz"]
...
class TestClean(unittest.TestCase):
    def _run(self, args: List[str]):
        proc = subprocess.Popen(
            ...
            env={
                ...
                )  # so it can find opuscleaner code
            },
    ...
    def test_simple(self):
        ...
        config = {
            "version": 1,
            "files": FILES,
            "filters": [{"filter": "deescape_tsv", "parameters": {}, "language": None}],
        }
    ...
    def test_filter_fail(self):
        ...
        config = {
            ...
            "files": FILES,
            "filters": [
                {"filter": "fail", "parameters": {"EXITCODE": "42"}, "language": "de"}
            ],
        }
        with NamedTemporaryFile(mode="w", dir=TEST_CWD / "data/train-parts") as fh:
            json.dump(config, fh)
        ...
        with NamedTemporaryFile(mode="w", dir=TEST_CWD / "data/train-parts") as fconf:
            ...
            with TemporaryFile("w+b") as fdata:
                ...
                with ExitStack() as ctx:
                    fhs = [
                        ctx.enter_context(
                            gzip.open(TEST_CWD / "data/train-parts" / filename)
                        )
            ...
            for mode, args in SCENARIOS.items():
                with self.subTest(mode=mode), NamedTemporaryFile(mode="r+") as trace_fh:
                    out, err, retval = self._run(
                        ["--trace", trace_fh.name, "--time", *args, fh.name]
                    )
                    ...
                    time_records = [
                        ...
                        for record in accumulate_records(
                            parse_record(line) for line in trace_fh
                        )
                        if "time" in record
                    ]
                    ...
                    for record in time_records:
                        self.assertEqual(
                            set(record["time"].keys()), {"user", "real", "sys"}
                        )
